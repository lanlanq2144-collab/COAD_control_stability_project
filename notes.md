...
Q1. 실제 암 발견할때는 저렇게 왜 안함?(소변이라던가 그런거 검사할때 대략 위험군이다 정도는 알려줄수있는거아냐?)
좋은 질문이에요. 이미 하고 있는데 생각보다 복잡해요.
...
1. 비용
   → Cologuard 한 번에 $600~$700
   → 보험 적용 안 되는 경우 많음

2. 위양성 문제
   → "암 같다"고 나왔는데 실제로는 아닌 경우
   → 불필요한 대장내시경, 수술로 이어질 수 있음
...
네 연구랑 연결하면
CDH3, CLDN1, ETV4, KRT80이 혈액이나 대변에서도 검출 가능한지 확인되면 이런 검사 패널에 들어갈 수 있어요. 지금 네가 한 게 그 첫 번째 단계예요.


입력 내용: 왤케 비싸
왤케 비싸
...
Cologuard 특허 = Exact Sciences 독점
→ 경쟁자가 없으니 가격 내릴 이유 없음
...
2010년대 → 유전자 전체 시퀀싱 $10,000
지금     → $200 이하
10년 후  → 더 내려갈 거예요
Illumina 같은 회사들이 시퀀싱 비용을 계속 낮추고 있어서, 네가 연구자로 일할 시점에는 지금보다 훨씬 보편화되어 있을 거예요.
그게 이 분야가 지금 제일 빠르게 성장하는 이유이기도 해요.





Q2. 바이오마커일 가능성을 높이기위해 어떻게 해야하는가?
> 다른 데이터를 독립적으로 분석한다: GEO에서 GSE156451를 가져와서 같은 방식으로 분석
(Claude Code로 실행)
GSE156451 외부 검증 결과 (results/external_validation/)
데이터: 중국 우한대학 CRC 환자 72명, 종양+정상 쌍체(paired) 샘플, FPKM
유전자    log₂FC    p-value    쌍체 p-value    유의성
CDH3    +3.16    1.1×10⁻³⁶    7.6×10⁻³⁴    ★★★
CLDN1    +3.35    7.8×10⁻³⁵    5.5×10⁻³³    ★★★
ETV4    +4.08    2.8×10⁻⁴⁴    2.9×10⁻³⁷    ★★★
KRT80    +2.61    1.5×10⁻³¹    1.9×10⁻²⁸    ★★★
로 여기에서도 매우 신뢰 가능하다는 결과가 나옴
